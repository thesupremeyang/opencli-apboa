import { cli, Strategy } from '@jackwener/opencli/registry';
import { ArgumentError, CommandExecutionError, EmptyResultError } from '@jackwener/opencli/errors';
import { apiFetch, authHeaders } from './utils.js';

const DEFAULT_BASE = 'http://117.72.185.237:3000';

cli({
  site: 'apboa',
  name: 'knowledge-update',
  description: '更新知识库配置',
  domain: '117.72.185.237',
  strategy: Strategy.PUBLIC,
  browser: false,
  access: 'write',
  args: [
    { name: 'id', required: true, positional: true, help: '知识库 ID' },
    { name: 'name', type: 'string', default: '', help: '新名称（留空则不更新）' },
    { name: 'description', type: 'string', default: '', help: '新描述（留空则不更新）' },
    { name: 'chunkSize', type: 'int', default: -1, help: '新分块大小（-1 表示不更新）' },
    { name: 'topK', type: 'int', default: -1, help: '新检索数量（-1 表示不更新）' },
    { name: 'enabled', type: 'string', default: '', help: '启用状态: true / false' },
    { name: 'base', type: 'string', default: DEFAULT_BASE, help: '平台地址' },
  ],
  columns: ['success', 'knowledgeId', 'name', 'updated'],
  func: async (args) => {
    const id = String(args.id || '').trim();
    if (!id) throw new ArgumentError('知识库 ID 不能为空');

    const base = String(args.base || DEFAULT_BASE).replace(/\/+$/, '');

    // Fetch existing knowledge base
    let existing;
    try {
      const data = await apiFetch(base, `/api/knowledge/config/${id}`);
      existing = data?.data || data;
    } catch (e) {
      throw new EmptyResultError('apboa knowledge-update', `未找到 ID 为 ${id} 的知识库配置`);
    }

    if (!existing || !existing.id) {
      throw new EmptyResultError('apboa knowledge-update', `未找到 ID 为 ${id} 的知识库配置`);
    }

    // Build update payload
    const payload = { ...existing, id };

    const newName = String(args.name || '').trim();
    if (newName) payload.name = newName;

    const newDesc = String(args.description || '').trim();
    if (newDesc) payload.description = newDesc;

    const newChunkSize = Number(args.chunkSize);
    if (newChunkSize > 0) {
      payload.retrievalConfig = { ...payload.retrievalConfig, chunkSize: newChunkSize };
    }

    const newTopK = Number(args.topK);
    if (newTopK > 0) {
      payload.retrievalConfig = { ...payload.retrievalConfig, topK: newTopK };
    }

    const newEnabled = String(args.enabled || '').trim().toLowerCase();
    if (newEnabled === 'true') payload.enabled = true;
    if (newEnabled === 'false') payload.enabled = false;

    const url = `${base}/api/knowledge/config`;
    const headers = {
      ...authHeaders(),
      'Content-Type': 'application/json',
    };

    let resp;
    try {
      resp = await fetch(url, {
        method: 'PUT',
        headers,
        body: JSON.stringify(payload),
      });
    } catch (e) {
      throw new CommandExecutionError(`更新知识库失败: ${e.message}`);
    }

    if (!resp.ok) {
      const text = await resp.text().catch(() => '');
      throw new CommandExecutionError(`更新知识库失败: ${resp.status} ${resp.statusText} - ${text}`);
    }

    const updatedFields = [];
    if (newName) updatedFields.push('name');
    if (newDesc) updatedFields.push('description');
    if (newChunkSize > 0) updatedFields.push('chunkSize');
    if (newTopK > 0) updatedFields.push('topK');
    if (newEnabled) updatedFields.push('enabled');

    return [{
      success: true,
      knowledgeId: id,
      name: payload.name,
      updated: updatedFields.join(', ') || 'none',
    }];
  },
});
