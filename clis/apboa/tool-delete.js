...
cli({
  ...
  func: async (args) => {
    ...
    try {
      ...
      throw new CommandExecutionError(`删除工具失败: ${e.message}`);
    }

    if (!resp.ok) {
      const text = await resp.text().catch(() => '');
      throw new CommandExecutionError(`删除工具失败: ${resp.status} ${resp.statusText} - ${text}`);
    }
